...
RSpec.describe Henitai::Runner do
  ...
  describe "debt cleanups (targeted specs)" do
    ...
    describe "recipe fast path validation" do
      it "skips mutant generation when all survivors have cached activation recipes and passes executed mutants" do
        Dir.mktmpdir do |dir|
          FileUtils.mkdir_p(File.join(dir, "lib"))
          File.write(File.join(dir, "lib/sample.rb"), "class Sample; end\n")
    ...
    describe "#resolve_subjects" do
      it "defaults source_files to self.source_files" do
        runner = described_class.new(config: build_config)
      ...
      it "applies pattern on the resolver with the correct pattern expression" do
    ...
    describe "mutants_for and static_filter workflow" do
      it "calls static_filter.apply with the generated mutants and config" do
        config = build_config(reporters: [])
        runner = described_class.new(config:)
      ...
      it "applies survivor selection in survivor rerun" do
    ...
    describe "reporters and progress_reporter" do
    ...
    describe "#source_provider" do
      it "returns a lambda that reads files and caches their content" do
        runner = described_class.new(config: build_config)
    ...
    describe "#reject_excluded" do
      it "returns the files immediately and does not normalize paths when excludes is empty" do
